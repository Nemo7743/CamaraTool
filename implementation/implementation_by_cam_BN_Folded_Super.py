...
def gray_world_awb(img):
    """
    灰度世界自動白平衡演算法 (Gray World Algorithm)
    假設場景的平均顏色應該是灰色的 (R=G=B)
    """
    # 避免修改原圖，建立副本並轉為 float32 以進行計算
...
try:
    while True:
        ...
        # 顯示 AWB 狀態 (預設開啟)
        cv2.putText(frame, f"FPS: {int(fps)} | {mode_str} | AWB: ON", (10, 120), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)

        # 繪製 ROI
        cv2.rectangle(frame, (x1, y1), (x2, y2), (255, 255, 0), 2)
        
        cv2.imshow('Fused Model Inference (AWB On)', frame)
